...
int reserved_or_fetch(page_fault_t error, uint32_t cr2)
{
    ...
    if (error.fetch) {
        DPRINTF("Page fault caused by invalid instruction fetch at %lx\n", cr2);
        return -1;
    }
    // ideally we will never have these
    if (error.reserved) {
        DPRINTF("Page fault caused by reserved bit set to 1 at %lx\n", cr2);
        return -1;
    }
    return 0;
}
...
int perm_present(page_fault_t error, entry_t table, entry_t dir, uint32_t cr2)
{
    ...
    if (error.user && !is_user(&table, &dir)) {
        DPRINTF("User proc tried to access kernel memory at %lx\n", cr2);
        return -1;
    }
    // Whatever they want legitimately isn't there
    if (!table.present) {
        DPRINTF("Process tried to access not present page at %lx\n", cr2);
        return -1;
    }
    return 0;
}
...
int vm_resolve_pagefault(ppd_t* ppd, uint32_t cr2, int error_code)
{
    ...
    if (vm_get_address(ppd, (void*)cr2, &table, &dir) < 0) {
        DPRINTF("Getting the page table or dir failed at %lx\n", cr2);
        return -1;
    }
    ...
    if (error.write <= table->write) {
        ...
        DPRINTF("Access to %lx faulted, but seems to be cool now\n", cr2);
        return 0;
    }
    if (!table->zfod) {
        DPRINTF("Process tried to write to read only page at %lx\n", cr2);
        return -1;
    }
    // error { fetch = 0, reserved = 0}
